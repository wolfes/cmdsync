...
(defn wrap-failsafe 
  "Wrap request handler in top-level try/catch."
  [handler]
  (fn [req]
    (try (handler req)
         (catch Exception e
...
(defn wrap-request-logging-in-dev
  "Wrap handler to log request details."
  [handler]
...
(defn wrap-reload-in-dev 
  "Reload templates before handling each request in dev."
  [handler]
  (if (= (conf/cfg :profile) :dev)
    (fn [req]
...
(defn json-response 
  "If response has a :body, jsonify it.  Also fixes header and :status 200."
  [resp]
...
;; Macros for json'ifying a handler's response. 
